 ...
 * 2. Builds and starts watermelon-sync server
...
async function buildWatermelonSync() {
  ...
  const syncDir = path.join(ROOT_DIR, "watermelon-sync");
  ...
  if (!existsSync(path.join(syncDir, "dist", "main.js"))) {
    throw new Error("watermelon-sync build did not produce dist/main.js");
  }
...
async function startWatermelonSync() {
  ...
  const proc = spawn("node", ["dist/main.js"], {
    cwd: path.join(ROOT_DIR, "watermelon-sync"),
    env: {
      ...process.env,
      ...
      S3_BUCKET: "watermelon-sync-test",
  ...
  proc.stdout.on("data", (data) => {
    if (process.env.VERBOSE) {
      console.log(`[watermelon-sync] ${data.toString().trim()}`);
    }
  });

  proc.stderr.on("data", (data) => {
    if (process.env.VERBOSE) {
      console.error(`[watermelon-sync] ${data.toString().trim()}`);
    }
  });

  proc.on("error", (err) => {
    error(`watermelon-sync error: ${err.message}`);
  });

  await waitForHealth(`http://localhost:${SYNC_PORT}/health`, 30000);
  log("watermelon-sync is ready");
