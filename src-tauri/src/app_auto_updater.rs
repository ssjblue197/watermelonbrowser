...
This module provides comprehensive self-update functionality for the WaterMelon Browser application
...
impl AppAutoUpdater {
  ...
  pub async fn check_for_updates(
    ...
    if self.should_update(&current_version, &latest_release.tag_name, is_nightly) {
      ...
      let release_page_url = format!(
        "https://github.com/ssjblue197/watermelonbrowser/releases/tag/{}",
        latest_release.tag_name
      );
  ...
  async fn fetch_app_releases(
    ...
    let url = "https://api.github.com/repos/ssjblue197/watermelonbrowser/releases?per_page=100";
    let response = self
      .client
      .get(url)
  ...
  fn is_deb_repo_configured() -> bool {
    Path::new("/etc/apt/sources.list.d/watermelonbrowser.list").exists()
  }

  /// Check if an RPM repository is configured (yum/dnf or zypper)
  #[cfg(target_os = "linux")]
  fn is_rpm_repo_configured() -> bool {
    Path::new("/etc/yum.repos.d/watermelonbrowser.repo").exists()
      || Path::new("/etc/zypp/repos.d/watermelonbrowser.repo").exists()
  }
  ...
  async fn install_update(
    ...
    #[cfg(target_os = "macos")]
    {
      ...
      // Clean up old "WaterMelon Browser.app" if it exists (from before the project rename)
      if let Some(parent_dir) = current_app_path.parent() {
        let old_app_path = parent_dir.join("WaterMelon Browser.app");
        if old_app_path.exists() && old_app_path != current_app_path {
          log::info!(
            "Removing old 'WaterMelon Browser.app' from: {}",
            old_app_path.display()
          );
          if let Err(e) = fs::remove_dir_all(&old_app_path) {
            log::warn!("Warning: Failed to remove old 'WaterMelon Browser.app': {e}");
          } else {
            log::info!("Successfully removed old 'WaterMelon Browser.app'");
          }
        }
      }
...
mod tests {
  ...
  fn test_platform_specific_download_urls() {
    ...
    let all_assets = vec![
      ...
      // Linux assets
      AppReleaseAsset {
        name: "watermelonbrowser_0.1.0_amd64.deb".to_string(),
        browser_download_url: "https://example.com/amd64.deb".to_string(),
        size: 12345,
      },
      AppReleaseAsset {
        name: "watermelonbrowser-0.1.0-1.x86_64.rpm".to_string(),
