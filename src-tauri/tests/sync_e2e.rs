    ...
    Ok(response) => {
      if !response.status().is_success() {
        panic!(
          ...
          Server URL: {}\n\
          Please ensure:\n\
          1. MinIO is running (docker compose up -d in watermelon-sync/)\n\
          2. watermelon-sync server is running (cd watermelon-sync && pnpm start:dev)\n\
...
async fn test_encrypted_profile_sync() {
  use watermelonbrowser_lib::sync::encryption::{
