...
async fn test_wireguard_tunnel_init() {
  ...
  use watermelonbrowser_lib::vpn::{VpnTunnel, WireGuardTunnel};
...
async fn test_tunnel_manager() {
  use watermelonbrowser_lib::vpn::{TunnelManager, VpnStatus, VpnTunnel};
  ...
  impl VpnTunnel for MockTunnel {
    async fn connect(&mut self) -> Result<(), watermelonbrowser_lib::vpn::VpnError> {
      self.connected = true;
      Ok(())
    }

    async fn disconnect(&mut self) -> Result<(), watermelonbrowser_lib::vpn::VpnError> {
...
async fn cleanup_runtime() {
  let _ = watermelonbrowser_lib::proxy_runner::stop_all_proxy_processes().await;
  let _ = watermelonbrowser_lib::vpn_worker_runner::stop_all_vpn_workers().await;
...
async fn run_proxy_feature_suite(
  ...
  let vpn_worker = watermelonbrowser_lib::vpn_worker_runner::start_vpn_worker(vpn_id)
    .await
  ...
  let stats_file = watermelonbrowser_lib::app_dirs::cache_dir()
    .join("traffic_stats")
  ...
  watermelonbrowser_lib::vpn_worker_runner::stop_vpn_worker(&vpn_worker.id)
    .await
    .map_err(|error| error.to_string())?;
  Ok(())
}
...
async fn test_wireguard_traffic_flows_through_watermelon_proxy(
  ...
  {
    let storage = watermelonbrowser_lib::vpn::VPN_STORAGE.lock().unwrap();
