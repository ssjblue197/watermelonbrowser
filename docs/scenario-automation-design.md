...
- Cho phép người dùng dựng kịch bản tự động hóa browser (RPA) chạy trên profile
  Wayfern/Camoufox: điều hướng, đọc, tương tác, rẽ nhánh/vòng lặp, AI, hẹn giờ, xoay profile.
...
Rust backend  ── scenario/ ───────────────────────────────────┐
  Scheduler  →  Executor  →  ActionExecutor ─► dispatch_tool_call (mcp_server.rs)
       ...
       ▼
  Browser engines: Wayfern (CDP) · Camoufox (WebDriver BiDi)
```
...
| CDP/BiDi cho tool mới (tab/upload/key) | `wayfern_manager` (CDP) · BiDi layer trong `mcp_server.rs` |
...
- Tích hợp Synchronizer (Wayfern) làm block "mirror sang follower".
