...
| **Proxy worker** | Binary Rust riêng `watermelon-proxy` (tiến trình detached) |
| **Sync server** | NestJS (`watermelon-sync/`) — self-hostable, lưu trên S3-compatible storage |
| **Browser engines** | Cloak (Chromium, seed-based) + Camoufox (Firefox-based) |
...
Mỗi profile lưu: `id` (UUID), `name`, `browser` (`cloak`|`camoufox`), `version`, `release_type`
...
(`camoufox_config`/`cloak_config`), `dns_blocklist`, `proxy_bypass_rules[]`, trạng thái sync
...
- **Rail nav** (thanh dọc 40px bên trái): logo Watermelon (easter egg: click 5 lần), 6 mục —
...
1. **Welcome Dialog** 4 bước: Intro (feature grid) → License (chấp nhận điều khoản thương mại) →
   Permissions (xin quyền micro/camera, có thể bỏ qua) → Setup (tải browser + tạo profile đầu tiên).
...
| Tính năng | Mô tả |
|---|---|
| **Tạo profile** | Chọn browser (Camoufox/Cloak) + version + release type, gán proxy/VPN, group, extension group, DNS blocklist, đặt mật khẩu, launch hook. Trước khi tạo, hệ thống **validate proxy/VPN thực sự hoạt động** (`validate_profile_network`) — proxy chết hoặc hết hạn (402) sẽ chặn tạo. |
...
2. **Spawn proxy worker** — `watermelon-proxy` detached, cấp phát port `127.0.0.1:0`, ghi `ProxyConfig`
   xuống đĩa, chờ worker bind & cập nhật `local_url` (timeout ~4s). Detach bằng `setsid()` (Unix) /
   `DETACHED_PROCESS` (Windows), set priority cao.
3. **VPN worker** (nếu profile có `vpn_id`) — spawn `watermelon-proxy vpn-worker`, dựng tunnel WireGuard →
   SOCKS5 cục bộ; watermelon-proxy chain qua SOCKS5 này.
...
### 5.2 Cloak (Chromium) — `cloak_manager.rs`
- Điều khiển qua **CDP** chuẩn (`--remote-debugging-port`), chờ CDP ready (tối đa ~60s).
- Fingerprint sinh từ **seed số** `--fingerprint=<seed>` (10000–99999) cùng vài cờ `--fingerprint-*`;
  binary tự sinh GPU/screen/hardware từ seed lúc launch — **không** inject runtime qua CDP.
- `randomize_seed_on_launch`: sinh seed mới mỗi lần launch và lưu lại profile.
- Proxy truyền qua cờ command-line. Extensions qua `--load-extension`.
- Không cần token cloud, không terms-gate; nói CDP Chromium chuẩn nên dùng chung path automation với MCP.
...
(aaOffset). Hỗ trợ **OS spoofing** (windows/macos/linux).
...
### Proxy worker cục bộ `watermelon-proxy` (`proxy_server.rs`, `bin/proxy_server.rs`)
- Mỗi profile launch spawn một worker **detached riêng** (1 file log/worker tại `$TMPDIR/watermelon-proxy-<id>.log`).
...
- **Chuỗi**: Browser → watermelon-proxy (port A) → SOCKS5 của VPN worker (port B) → tunnel WireGuard.
...
Edge** (→ cloak). Import tên, đường dẫn, cookies (giải mã nếu cần).
...
- **DNS blocklist** (`dns_blocklist.rs`): chặn quảng cáo/tracker theo **từng profile**; blocklist cache,
  ...
  thực thi tại watermelon-proxy (trả 403).
...
- **Unlock khi launch** (`profile/password.rs`): file `.watermelon-pw-verify` kiểm tra mật khẩu trước khi chạm
...
- **Trả phí (Pro/Team)**: cloud sync (mirror đa thiết bị), team profile locking, **fingerprint
  cross-OS + MCP automation**, cloud proxy & chọn vị trí, tăng giới hạn profile.
...
### Sync engine (`sync/`)
- **Hai cơ chế** lưu lên S3-compatible (Watermelon cloud hoặc `watermelon-sync` self-host):
...
- **Auto-update app** (`app_auto_updater.rs`): kiểm tra version mới (stable/nightly), tải & chuẩn bị,
  restart; có thể tắt (`disable_auto_updates`). Hỗ trợ repo Linux (deb/rpm tại `repo.watermelonbrowser.com`).
- **Cập nhật browser** (`auto_updater.rs`, `version_updater.rs`): kiểm tra version mới của Camoufox/Cloak,
  cập nhật profile, dọn version cũ; background task định kỳ, có thể trigger thủ công.
- **Default browser** (`default_browser.rs`): đặt Watermelon làm trình duyệt mặc định; khi click link →
...
- **Logs**: GUI log tại thư mục log của OS (rotate 5MB, KeepAll); worker log tại `$TMPDIR`; Camoufox stderr
  riêng. App data: `WatermelonBrowser` (release) / `WatermelonBrowserDev` (debug), override bằng `WATERMELONBROWSER_DATA_ROOT`.
...
| 2 engine: Camoufox (Firefox) / Cloak (Chromium) | ✅ Free |
