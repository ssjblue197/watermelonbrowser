  ...
  if let Some(proxy_matches) = matches.subcommand_matches("proxy") {
    if let Some(start_matches) = proxy_matches.subcommand_matches("start") {
      ...
      if let Some(id) = stop_matches.get_one::<String>("id") {
        ...
        let configs = watermelonbrowser_lib::proxy_storage::list_proxy_configs();
    ...
    if action == "start" {
      ...
      let config = if let Some(path) = config_path {
        ...
        match std::fs::read_to_string(path) {
          Ok(content) => match serde_json::from_str::<
            watermelonbrowser_lib::vpn_worker_storage::VpnWorkerConfig,
          >(&content)
          {
            Ok(config) => {
              log::info!(
        ...
        let storage_dir = watermelonbrowser_lib::proxy_storage::get_storage_dir();
      ...
      match config.vpn_type.as_str() {
        "wireguard" => {
          ...
          let server =
            watermelonbrowser_lib::vpn::socks5_server::WireGuardSocks5Server::new(wg_config, port);
          if let Err(e) = server
